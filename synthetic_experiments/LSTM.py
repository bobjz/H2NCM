...
for alpha in [0,1e-4,1e-3,1e-2,1e-1,1]:
    ...
    for repeat in range(3):
        for test_split in range(5):
            ...
            hidden_dim=[8,16]
            dropout=[0,0.2]
            hyper_params=[hidden_dim,num,dropout]
            ...
            for i in range(len(list_params)):
                ...
                for val_split in range(3):
                    ...
                    model=LSTM(input_size=2,latent_size=int(params[0]),num_layers=int(params[1]),dropout=params[2])
            ...
            model=LSTM(input_size=2,latent_size=int(best_param[0]),num_layers=int(best_param[1]),dropout=best_param[2])
            train_h,val_h,test_h=train_model(model,alpha,beta,train,val,test,epochs=50,lr=2*1e-3,\
                                             device=device,path=f"LSTM2_{alpha}_{repeat}_{test_split}.pth")
    ...
    np.save(f"LSTM2_a{alpha}_pred.npy",rmse)
    np.save(f"LSTM2_a{alpha}_causal.npy",er)
    np.save(f"LSTM2_a{alpha}_best_params.npy",best_param_list)
    print(f"LSTM2_{alpha} RMSE {np.mean(rmse)}")
    print(f"LSTM2_{alpha} Classification Error Rate {np.sort(er)}")
