...
for alpha in [0,1e-4,1e-3,1e-2,1e-1,1]:
        ...
        for test_split in range(5):
            ...
            for i in range(len(list_params)):
                ...
                for val_split in range(3):
                    ...
                    model=TCN_wrapper(3,10, num_channels=[int(params[1])]*int(params[0]),\
            ...
            model=TCN_wrapper(3,10, num_channels=[int(best_param[1])]*int(best_param[0]),\
            ...
            train_h,val_h,test_h=train_model(model,alpha,beta,train,val,test,epochs=50,lr=2*1e-3,\
                                             device=device,path=f"TCN2_{alpha}_{repeat}_{test_split}.pth")
    ...
    np.save(f"TCN2_a{alpha}_pred.npy",rmse)
    np.save(f"TCN2_a{alpha}_causal.npy",er)
    np.save(f"TCN2_a{alpha}_best_params.npy",best_param_list)
    print(f"TCN2_{alpha} RMSE {np.mean(rmse)}")
    print(f"TCN2_{alpha} Classification Error Rate {np.sort(er)}")
