...
for alpha in [0,1e-4,1e-3,1e-2,1e-1,1]:
    ...
    np.save(f"UVA_reduced_a{alpha}_pred.npy",rmse)
    np.save(f"UVA_reduced_a{alpha}_causal.npy",er)
    np.save(f"UVA_reduced_a{alpha}_best_params.npy",best_param_list)
    print(f"UVA_reduced_{alpha} RMSE {np.mean(rmse)}")
    print(f"UVA_reduced_{alpha} Classification Error Rate {np.sort(er)}")
