...
for alpha in [0,1e-4,1e-3,1e-2,1e-1,1]:
        ...
        for test_split in range(5):
            ...
            for i in range(len(list_params)):
                ...
                for val_split in range(3):
                    ...
                    model=LPSC_LSTM(closure_input_sizes,latent_size=int(params[2]),mlp_size=int(params[1]),num_hidden_layer=int(params[0]))
            ...
            model=LPSC_LSTM(closure_input_sizes,latent_size=int(best_param[2]),mlp_size=int(best_param[1]),num_hidden_layer=int(best_param[0]))
            train_h,val_h,test_h=train_model(model,alpha,beta,train,val,test,epochs=50,lr=10*1e-3,device=device)
            model.turn_on_closure()
            train_h,val_h,test_h=train_model(model,alpha,beta,train,val,test,epochs=50,lr=10*1e-3,\
                                             device=device,path=f"LPSC_{alpha}_{repeat}_{test_split}.pth")
    ...
    np.save(f"LPSC_a{alpha}_pred.npy",rmse)
    np.save(f"LPSC_a{alpha}_causal.npy",er)
    np.save(f"LPSC_a{alpha}_best_params.npy",best_param_list)
    print(f"LPSC_{alpha} RMSE {np.mean(rmse)}")
    print(f"LPSC_{alpha} Classification Error Rate {np.sort(er)}")
