...
for alpha in [0,1e-4,1e-3,1e-2,1e-1,1]:
        ...
        for test_split in range(5):
            ...
            for i in range(len(list_params)):
                ...
                for val_split in range(3):
                    ...
                    #print(total_params)
                    train_h,val_h,test_h=train_trans(model,alpha,beta,train,val,test,epochs=50,lr=2*1e-3,device=device)
            ...
            train_h,val_h,test_h=train_trans(model,alpha,beta,train,val,test,epochs=50,lr=2*1e-3,\
                                            device=device,path=f"Tran2_{alpha}_{repeat}_{test_split}.pth")
    ...
    np.save(f"Tran2_a{alpha}_pred.npy",rmse)
    np.save(f"Tran2_a{alpha}_causal.npy",er)
    np.save(f"Tran2_a{alpha}_best_params.npy",best_param_list)
    print(f"Tran2_{alpha} RMSE {np.mean(rmse)}")
    print(f"Tran2_{alpha} Classification Error Rate {np.sort(er)}")
