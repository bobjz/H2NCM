...
for alpha in [0,1e-4,5e-4,1e-3,5e-3,1e-2,5e-2,1e-1,5e-1,1]:
    ...
    for repeat in range(2):
        for test_split in range(5):
            ...
            train_h,val_h,test_h=train_model(model,alpha,beta,train,val,test,epochs=50,lr=10*1e-3,\
                                             device=device,path=f"LSTM_{alpha}_{repeat}_{test_split}.pth")
    ...
    np.save(f"LSTM_a{alpha}_pred.npy",rmse)
    np.save(f"LSTM_a{alpha}_causal.npy",er)
    np.save(f"LSTM_a{alpha}_best_params.npy",best_param_list)
    print(f"LSTM_{alpha} RMSE {np.mean(rmse)}")
    print(f"LSTM_{alpha} Classification Error Rate {np.sort(er)}")
